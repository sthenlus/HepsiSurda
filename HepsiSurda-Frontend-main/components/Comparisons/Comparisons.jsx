...
import {
  ...
  updateComparison,
} from "../../network/lib/comparison"; // Path'i güncellediğinizden emin olun
...
const Comparisons = () => {
  ...
  const [userId, setUserId] = useState(null); // userId'yi state olarak tutuyoruz

  useEffect(() => {
    // `localStorage`'ı burada çağırıyoruz
    if (typeof window !== "undefined") {
      const storedUserId = localStorage.getItem("user_id");
      setUserId(storedUserId);
    }
  }, []); // Bu sadece bir kez, komponent yüklendikten sonra çalışacak
  ...
  }, [userId]); // `userId` değiştiğinde tekrar fetchComparisons çalışır
